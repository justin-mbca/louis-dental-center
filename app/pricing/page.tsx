...
export default function PricingPage() {
  return (
    <main>
      ...
      <section className="px-5 py-20 sm:px-8 lg:px-12"><div className="mx-auto grid max-w-[1200px] gap-5 md:grid-cols-2">{items.map(([title,text])=><div key={title} className="premium-card rounded-3xl p-7"><h2 className="text-2xl font-bold text-[#09233f]">{title}</h2><p className="mt-3 text-lg leading-8 text-slate-600">{text}</p></div>)}</div><div className="mx-auto mt-10 max-w-[1200px] rounded-3xl bg-[#f6f1e8] p-8 text-center"><h2 className="display-font text-4xl font-bold text-[#09233f]">Request a custom quote.</h2><p className="mt-4 text-lg leading-8 text-slate-700">Contact the lab team for pricing based on file quality, materials, due date, and complexity.</p><Link href="/contact" className="button-primary mt-7">Contact for Quote</Link></div>
      <div className="mx-auto mt-6 max-w-[1200px] rounded-2xl border border-slate-200 bg-slate-50 p-5 text-center text-base leading-7 text-slate-500">
